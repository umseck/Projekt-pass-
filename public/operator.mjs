...
export async function operatorPage(route,id,ctx){
 ...
 if(route==='admin'){
   ...
   shell(`<section class="admin-title"><span class="eyebrow">Betreiberverwaltung</span><h1>Ihre Betriebe.<br>Alles im Blick.</h1><p class="intro">Betriebsprofile und Zugänge für PROJEKTPASS verwalten.</p><a class="btn olive spaced" href="#business-new">+ Betrieb anlegen</a><div class="stats"><div><strong>${data.companies.length}</strong><span>Betriebe</span></div><div><strong>${data.companies.filter(c=>c.active).length}</strong><span>Aktive Zugänge</span></div></div><div class="search"><label class="sr-only" for="business-search">Betriebe suchen</label><input id="business-search" type="search" placeholder="Firma, Ansprechpartner oder E-Mail"></div><div id="businesses"></div></section>`);
 ...
 const fresh=route==='business-new';
 ...
 bindForm('#operator-business',async form=>{
   ...
   const saved=await api(fresh?'operator_create':'operator_save',{id:business.id,version:business.version,profile:next});
   setDirty(false);business.version=saved.version;business.profile=saved.profile;
   if(fresh){go('business/'+saved.id);notify('Betrieb mit 20 Projektpässen angelegt. Jetzt den Zugang einrichten.');}
